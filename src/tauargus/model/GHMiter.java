...
//NT = 0
//Open Temp + "\proto003" For Input As #1
//Line Input #1, Hs
//With TableSetStruct(TabNo)
// For i = 1 To 11
//  HS1 = GetToken(Hs, False)
//  If HS1 = "" Then GoTo FOUT2
//  n = Val(HS1)
//  NT = NT + n
//  .GhMiterRatio(i) = .GhMiterRatio(i) + n
//  Next i
//End With
//If NT <> 0 Then
// SDCMsgBox "Some (" + TrimIStr(NT) + ") cells could not be fully protected" + vbCrLf + _
//           "See report file for details", vbInformation
//End If
//While Not EOF(1)
// Line Input #1, Hs
// If Trim(Hs) <> "" Then GoTo FOUT2
//Wend
//Close #1
//TestProto003 = True
//Exit Function
       ...
       while((Hs = in.readLine()) != null) {
         ...
         if ( aprioryPerc == 100 && tableSet.minTabVal == 0) {
            ...
            Regel = Regel + "  " + Hs;             
             ...
             if ( Double.parseDouble(minResp) == 0 && Double.parseDouble(maxResp) == 0) {
                X = 0;}
